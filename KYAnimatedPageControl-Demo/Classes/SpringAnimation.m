...
+(CAKeyframeAnimation *)createSpring:(NSString *)keypath duration:(CFTimeInterval)duration usingSpringWithDamping:(CGFloat)damping initialSpringVelocity:(CGFloat)velocity fromValue:(id)fromValue toValue:(id)toValue{
    ...
    NSMutableArray *values = [self animationValues:fromValue toValue:toValue usingSpringWithDamping:damping * dampingFactor initialSpringVelocity:velocity * velocityFactor duration:duration];
...
+(CGFloat)normalizeAnimationValues:(CGFloat)x usingSpringWithDamping:(CGFloat)damping initialSpringVelocity:(CGFloat)velocity{
